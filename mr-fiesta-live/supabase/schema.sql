...
-- CRM de contratos y agenda. Separado de events para no mezclar shows LIVE con reservas comerciales.
create table if not exists public.crm_events (
  id uuid primary key default gen_random_uuid(),
  cliente text not null check (char_length(trim(cliente)) between 2 and 160),
  agasajado text not null default '',
  ...
  adelanto numeric not null default 0 check (adelanto >= 0 and adelanto <= total),
  saldo numeric generated always as (greatest(total - adelanto, 0)) stored,
  drive_pdf_url text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
create index if not exists crm_events_date_idx on public.crm_events(fecha_evento, hora_inicio);
alter table public.crm_events enable row level security;
drop policy if exists "admins manage crm events" on public.crm_events;
